...
const AppConfig = {
	...
	lastfmApiKey: process.env.LASTFM_API_KEY as string,
	lastfmApiBaseUrl: config.get('lastfm.apiBaseUrl') as string,
	lastfmFrontendRedirectPath: config.get('lastfm.frontendRedirectPath') as string,
	lastfmBackfillMaxPages: config.get('lastfm.backfillMaxPages') as number,
	lastfmPollLimit: config.get('lastfm.pollLimit') as number,
	lastfmPollIntervalMs: config.get('lastfm.pollIntervalMs') as number
}
